...
def _context(target, request_id):
    """Bind target and immutable request identity inside the authenticated envelope."""
    if target not in SECRET_TARGETS or not isinstance(request_id, UUID):
        raise CryptographicError("Invalid credential handoff identity.")
    return b"credential-handoff-v1:" + target.encode("ascii") + b":" + request_id.bytes
...
class PublicHandoff:
    ...
    def seal(self, request_id, value):
        ...
        return TokenPublicKeyring([self.key]).encrypt(
            value,
            context=_context(self.target, request_id),
        )
...
class PrivateHandoff:
    ...
    def open(self, request_id, ciphertext):
