...
-- only, and never NULL: a payload without a name yields an empty name, as
-- the pure source suggestions do. Nothing here is an authorization: a row is
-- a fact about the parish source, and only the Administrator-only Portal
-- users page reads it. The narrow projection beneath it is the one
-- definition of a current Chairperson, shared with the reconciliation
-- owners; the restricted web role is granted this view and not that
-- projection, so the page can never redefine "current Chairperson" from the
-- source tables it reads elsewhere.
--
-- Address ownership is aggregated once per snapshot and address and joined,
-- rather than recomputed for every Chairperson row: the page reads this
-- under the work lock, so its cost is bounded by the snapshot's Members, not
-- by their product with the roster.
...
SELECT chair.snapshot_id,
       ...
       chair.ministry_duid,
       COALESCE(ministry.canonical::jsonb->>'name', '') AS ministry_name,
       chair.email,
       chair.publish_email,
       chair.roster_key,
       ...
       owners.member_duids AS address_members
  ...
  LEFT JOIN (
       SELECT om.snapshot_id,
              email.value->>'value' AS email,
              array_agg(DISTINCT (other.source_key)::bigint
                        ORDER BY (other.source_key)::bigint) AS member_duids
        ...
        WHERE other.canonical::jsonb->'schema_version'='1'::jsonb
        ...
        GROUP BY om.snapshot_id, email.value->>'value') owners
    ON owners.snapshot_id=chair.snapshot_id AND owners.email=chair.email;
