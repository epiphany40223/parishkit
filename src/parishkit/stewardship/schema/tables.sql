...
CREATE TABLE public.stewardship_limiter_health (
    ...
    admin_healthy_since timestamp with time zone,
    family_healthy_since timestamp with time zone,
    store_healthy_since timestamp with time zone,
    CONSTRAINT limiter_health_admin_window CHECK (admin_healthy_since IS NULL OR (observed_at IS NOT NULL AND admin_healthy_since <= observed_at)),
    CONSTRAINT limiter_health_family_window CHECK (family_healthy_since IS NULL OR (observed_at IS NOT NULL AND family_healthy_since <= observed_at)),
    CONSTRAINT limiter_health_store_window CHECK (store_healthy_since IS NULL OR (observed_at IS NOT NULL AND store_healthy_since <= observed_at)),
...
CREATE TABLE public.stewardship_family_mail_preparation (
    id uuid PRIMARY KEY,
    ...
    rehearsal_epoch_id uuid,
    CONSTRAINT family_mail_preparation_scope UNIQUE NULLS NOT DISTINCT
        (occurrence_id,rehearsal_epoch_id),
    CONSTRAINT family_mail_preparation_namespace CHECK (
        (mode='production' AND rehearsal_epoch_id IS NULL)
        OR (mode='testing' AND rehearsal_epoch_id IS NOT NULL))
);
CREATE INDEX family_mail_preparation_correlation
    ON public.stewardship_family_mail_preparation(correlation_id);
...
-- Retained preparation lineage is metadata, never another delivery identity.
CREATE TABLE public.stewardship_recovery_replacement (
    id uuid PRIMARY KEY,
    created_at timestamptz DEFAULT statement_timestamp() NOT NULL,
    actor_id uuid,
    correlation_id uuid NOT NULL,
