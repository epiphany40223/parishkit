...
This wrapper delegates to the installed `pk-validate-gcalendar-reservations`
command.

## Usage

```sh
pk-validate-gcalendar-reservations --config example-config.yaml --dry-run
```
...
Set `common.timezone` for the default local timezone. A
`calendars.timezone` value overrides it for this command.
